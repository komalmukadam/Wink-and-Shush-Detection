...
def detect(frame, faceCascade, eyesCascade):
   ...
   # gray_frame = cv2.equalizeHist(gray_frame)
    ...
    # if(len(faces)==0):
    #     faces= faceCascade.detectMultiScale(
    #         frame, 1.1,2 , 0|cv2.CASCADE_SCALE_IMAGE, (10, 10))
...
if __name__ == "__main__":
    ...
    eye_cascade = cv2.CascadeClassifier(cv2.data.haarcascades
                                      + 'haarcascade_eye.xml')
    FACES_CASCADE_NAME_2 = cv2.CascadeClassifier(cv2.data.haarcascades
                                                 + '"haarcascade_profileface.xml')
    EYES_CASCADE_NAME_2 = cv2.CascadeClassifier(cv2.data.haarcascades+'haarcascade_mcs_righteye.xml')
